...
def build_child_parent_map(certificates):
    """
    Builds a dictionary that maps each certificate to a list of its parent certificates.
    
    Args:
        certificates (list): A list of certificates.
    
    Returns:
        dict: A dictionary where the keys are certificates, and the values are lists
        of the parent certificates for each key.
    """
